...
// Nav agrupado por secciones. Cada item se filtra por su permiso.
const NAV_GROUPS = [
  {
    section: null,
    items: [
...
export default function Sidebar({ mobileOpen, onClose }) {
  ...
  // En el drawer movil siempre se muestra expandido (la contraccion es de escritorio).
  const slim = collapsed && !mobileOpen;

  // Grupos visibles segun permisos (oculta secciones sin items).
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex transform flex-col bg-navy-700
      ...
      >
            ...
            <div key={group.section || `g${gi}`}>
              {group.section &&
                (slim ? (
                  ...
                  <p className="px-3 pb-1 pt-4 text-xs font-semibold uppercase tracking-wider text-navy-300">
                    {group.section}
                  </p>
                ))}
        ...
        <button
          ...
          className={`hidden items-center border-t border-navy-600 px-5 py-4 text-sm font-medium
            text-navy-100 transition-colors hover:bg-navy-600 lg:flex
            ${slim ? 'justify-center' : 'gap-3'}`}
