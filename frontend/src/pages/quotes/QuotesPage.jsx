      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map(q => {
            ...
            return (
              <div key={q.id} style={{ background: '#151d2e', border: '1px solid #1e2d45', borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    ...
                    <p style={{ margin: '2px 0', fontSize: 14, fontWeight: 600, color: '#e2e8f0' }}>{q.client_name || '—'}</p>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <button onClick={() => handleDownloadPDF(q)} title="Descargar PDF" style={{ background: '#1e2d45', border: 'none', borderRadius: 7, padding: '7px 10px', cursor: 'pointer', color: '#10b981' }}><Download size={16} /></button>
                    <button onClick={() => navigate('/cotizaciones/' + q.id + '/editar')} style={{ background: '#1e2d45', border: 'none', borderRadius: 7, padding: '7px 10px', cursor: 'pointer', color: '#94a3b8' }}><Eye size={16} /></button>
                    <button onClick={() => navigate('/cotizaciones/' + q.id + '/editar')} style={{ background: '#1e2d45', border: 'none', borderRadius: 7, padding: '7px 10px', cursor: 'pointer', color: '#94a3b8' }}><Pencil size={16} /></button>
                    <button onClick={() => handleDelete(q.id)} style={{ background: '#1e2d45', border: 'none', borderRadius: 7, padding: '7px 10px', cursor: 'pointer', color: '#ef4444' }}><Trash2 size={16} /></button>
                  </div>
                </div>
              </div>
            );
